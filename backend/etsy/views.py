...
class EtsyOAuthCallbackView(APIView):
    def post(self, request):
        ...
        try:
            ...
            etsy_user_id = int(me.get("user_id", me.get("userId", 0)))
            conn.etsy_user_id = etsy_user_id or None
            shops = client.get_json(f"/users/{etsy_user_id}/shops", refresh_fn=refresh_fn)
            shop_results = shops.get("results") or shops.get("shops") or []
            if shop_results:
                first = shop_results[0]
                sid = first.get("shop_id") or first.get("shopId")
                conn.shop_id = int(sid) if sid is not None else None
...
class EtsyListingsView(APIView):
    def get(self, request):
        conn = EtsyConnection.objects.filter(user=request.user).first()
        ...
        results = listings_data.get("results") or []
        out = []
        ...
        try:
            for listing in results:
                ...
                try:
                    ...
                    listing = {**listing, "images": imgs.get("results") or imgs.get("images") or []}
